...
export async function POST(
  ...
  try {
    ...
    const result = await workflowOrchestrator.cancelWorkflow(id);
    return NextResponse.json(result);
  } catch (error: any) {
    logger.error({ err: error }, "API error");
    return NextResponse.json({ message: sanitizeErrorMessage(error, "Failed to cancel workflow") }, { status: 500 });
  }
}
