...
export async function DELETE(
  ...
  try {
    ...
    const existing = await storage.getDafAccount(id);
    if (!existing) return NextResponse.json({ message: "DAF account not found" }, { status: 404 });
    ...
    await storage.deleteDafAccount(id);
