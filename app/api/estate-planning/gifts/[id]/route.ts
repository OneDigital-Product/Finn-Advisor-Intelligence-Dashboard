...
export async function DELETE(_request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    const gift = await storage.getGiftHistoryEntry(id);
    if (!gift) return NextResponse.json({ message: "Gift not found" }, { status: 404 });
    const hasAccess = await verifyClientAccess(auth.session, gift.clientId);
    if (!hasAccess) return NextResponse.json({ message: "Access denied" }, { status: 403 });
    await storage.deleteGiftHistoryEntry(id);
