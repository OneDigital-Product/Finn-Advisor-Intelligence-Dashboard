...
export async function GET(_request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    const notes = await engine.generateMeetingBehavioralNotes(id);
    return NextResponse.json({ notes });
  } catch (err: any) {
    logger.error({ err: err }, "[Behavioral] Coaching notes error:");
    return NextResponse.json({ message: "Failed to generate coaching notes" }, { status: 500 });
  }
}
