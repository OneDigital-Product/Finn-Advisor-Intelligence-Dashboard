...
export async function DELETE(_request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    const existing = await storage.getFlpStructure(id);
    if (!existing) return NextResponse.json({ message: "FLP not found" }, { status: 404 });
    ...
    await storage.deleteFlpStructure(id);
