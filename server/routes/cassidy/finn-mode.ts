...
import { CASSIDY_SUBROUTER_URL, VALID_FINN_MODES, type FinnMode } from "../../lib/cassidy";
import { callSubAgentRouter } from "../../integrations/cassidy/webhook-client";
import { sanitizeForPrompt } from "../../openai";
import { chatCompletion } from "../../ai-core";
import crypto from "crypto";
import {
  storage, logger, requireAuth, rateLimiter,
  AuditLogger, AuditEventType, validateBody,
} from "./shared";
import { z } from "zod";

const finnModeSchema = z.object({
  message: z.string().min(1, "message is required"),
  selected_mode: z.enum(VALID_FINN_MODES.filter(m => m !== "conversation") as [string, ...string[]], {
    errorMap: () => ({ message: `selected_mode must be one of: ${VALID_FINN_MODES.filter(m => m !== "conversation").join(", ")}` }),
  }),
...
export function registerFinnModeRoutes(app: Express) {
  app.post("/api/cassidy/finn-mode", requireAuth, async (req: Request, res: Response) => {
    try {
      ...
      const body = validateBody(finnModeSchema, req, res);
      if (!body) return;

      const { message, selected_mode, client_name, conversation_id, client_id, page_context } = body;
      ...
      if (cassidyAvailable) {
        ...
        try {
          ...
          logger.warn({ err, jobId: job_id, mode: selected_mode }, "Sub-agent router call failed, trying OpenAI fallback");
          result = null;
        }
        ...
        if (result) {
          ...
          if (!finResponse || typeof finResponse !== "string" || finResponse.trim().length === 0) {
            logger.warn({ jobId: job_id, mode: selected_mode, responseKeys: Object.keys(result) }, "Sub-agent router returned invalid response, trying OpenAI fallback");
            finResponse = undefined;
          } else {
            if (!calledMode || typeof calledMode !== "string") {
              logger.warn({ jobId: job_id, mode: selected_mode }, "Sub-agent router response missing calledMode, using requested mode");
            }
      ...
      if (!finResponse) {
        try {
          ...
          let systemPrompt = "You are Finn, an AI copilot for financial advisors. Be concise, professional, and actionable.";
          ...
          logger.error({ err: fallbackErr, jobId: job_id, mode: selected_mode }, "OpenAI fallback also failed for finn-mode");
          await storage.db
            .update(cassidyJobs)
